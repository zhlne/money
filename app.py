...
with st.expander("📖 系統使用說明與進階參數指南 (初次使用請點此展開)", expanded=False):
    ...
    *   **實務操作：** 假設您明天以 100 元開盤價買進，請立刻（真的是立刻）在券商 APP 設定一筆「觸價單（洗價單）」，設定條件為：當價格 <= 90 元時，自動以市價（或跌停價）賣出。
    *   **實戰意義：** 絕不盯盤猶豫，絕不心存僥倖凹單。只要打到 -10% 停損線，代表這筆突破是「假突破」，請立刻把資金抽出來，尋找下一個機會。
    """)
            ...
            with st.container(border=True):
                ...
                with col2:
                    ...
                    fig.add_hline(y=est_sl_price, line_dash="dash", line_color="red", 
                                  annotation_text=f"10% 預估停損線 {est_sl_price:.2f}", annotation_position="bottom left")
